export default class BananaLeft {
  constructor(scene, x, y) {
      ...
      this.boxWidth = 110;  //Width of the score box
      ...
      const bananaTextY = this.y + 10;   //Vertical center for "POINTS"
      const textY = this.y + 30  //Adjusted score gap for score value

      // Create text for "POINTS"
      this.pointsText = this.scene.add.text(centerX , bananaTextY, 'BANANAS LEFT', {
          fontSize: '12px',
          fontFamily: 'Nunito',
          color: '#ff6600',
      }).setOrigin(0.5);
      ...
      this.countText = this.scene.add.text(centerX, textY, `${this.score}`, {
          fontSize: '20px',
  ...
  // Method to update the score display
    updateBananasLeft(count) {
      this.bananasLeft = count;
      this.countText.setText(`${this.bananasLeft}`);
    }
  }
